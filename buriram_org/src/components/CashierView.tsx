    ...
    <div className="space-y-8 animate-fade-in">
      ...
      {showAddMoneyModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/85 backdrop-blur-sm animate-fade-in">
          <div className="w-full max-w-md bg-[#0a0a0a] border border-white/10 rounded-3xl overflow-hidden shadow-2xl my-auto flex flex-col">
            <div className="shrink-0 p-4 border-b border-white/10 flex items-center justify-between bg-gradient-to-r from-[#140a2c] to-black">
              ...
              <button 
                ...
                className="p-1.5 hover:bg-white/5 rounded-lg text-zinc-500 hover:text-white transition-colors cursor-pointer text-xs"
              >
                ✕
              </button>
            </div>

            <form onSubmit={handleAddMoneySubmit} className="p-5 space-y-4">
              
              {/* TOP CENTER ALIGNED OFFICIAL MOBILE NUMBER */}
              <div className="text-center py-3 px-4 bg-gradient-to-r from-violet-950/40 via-zinc-900 to-violet-950/40 border border-violet-500/30 rounded-2xl space-y-1">
                <span className="text-[10px] font-mono text-zinc-400 uppercase tracking-widest font-bold block">
                  {method === "bKash" 
                    ? (isBengali ? "bKash সেন্ড মানি পার্সোনাল নাম্বার" : "bKash Send Money Personal Number") 
                    : (isBengali ? "Nagad সেন্ড মানি পার্সোনাল নাম্বার" : "Nagad Send Money Personal Number")}
                </span>
                <div className="flex items-center justify-center gap-2.5">
                  <span className="text-lg font-black font-mono text-amber-400 tracking-wider">
                    {method === "bKash" ? adminBksh : adminNagad}
                  </span>
                  <button
                    type="button"
                    onClick={() => {
                      navigator.clipboard.writeText(method === "bKash" ? adminBksh : adminNagad);
                      alert(isBengali ? "নাম্বার কপি করা হয়েছে!" : "Number copied!");
                    }}
                    className="px-2.5 py-1 bg-violet-600 hover:bg-violet-500 text-white rounded-lg transition-all cursor-pointer flex items-center gap-1 text-[10px] font-mono font-bold shadow-md"
                  >
                    <Copy className="w-3 h-3" />
              ...
              {/* Method Selector & How to Pay Button */}
              <div className="space-y-2">
                <div className="flex items-center justify-between">
                  <label className="text-[10px] font-mono text-zinc-400 uppercase tracking-widest font-black">
                    {isBengali ? "পেমেন্ট পদ্ধতি" : "PAYMENT METHOD"}
                  </label>
                  <button
                    type="button"
                    onClick={() => setShowHowToPayModal(true)}
                    className="text-[10px] font-mono font-bold text-violet-400 hover:text-violet-300 underline flex items-center gap-1 cursor-pointer"
                  >
                    <span>{isBengali ? "কীভাবে পেমেন্ট করবেন? (How to Pay)" : "How to Pay?"}</span>
                  </button>
                </div>
                ...
                <div className="grid grid-cols-2 gap-2.5">
                  <button
                    ...
                    className={`flex items-center justify-center gap-2 p-2.5 rounded-xl border transition-all cursor-pointer ${
                      method === "bKash" 
                        ? "bg-pink-500/10 border-pink-500 text-pink-400 font-bold shadow-md shadow-pink-950/20" 
                  ...
                  <button
                    ...
                    className={`flex items-center justify-center gap-2 p-2.5 rounded-xl border transition-all cursor-pointer ${
                      method === "Nagad" 
                        ? "bg-orange-500/10 border-orange-500 text-orange-400 font-bold shadow-md shadow-orange-950/20" 
                        : "bg-black/30 border-white/5 text-zinc-400 hover:text-zinc-300"
                    }`}
                  >
                    <NagadLogo />
              ...
              <div className="space-y-3 font-mono text-xs">
                <div>
                  <label className="block text-zinc-400 text-[10px] font-bold uppercase mb-1">
                    {isBengali ? "টাকার পরিমাণ (BDT)" : "Amount (BDT)"}
                  </label>
                  <input
                    type="number"
                    required
                    ...
                    placeholder="e.g. 100"
                    className="w-full p-2.5 bg-black/60 border border-white/10 rounded-xl text-white text-xs focus:outline-none focus:border-violet-600 focus:ring-1 focus:ring-violet-600 font-mono transition-all"
                  />
                </div>
                <div>
                  <label className="block text-zinc-400 text-[10px] font-bold uppercase mb-1">
                    {isBengali ? "আপনার মোবাইল নাম্বার" : "Your Mobile Number"}
                  </label>
                  <input
                    type="text"
                    required
                ...
                <div>
                  <label className="block text-zinc-400 text-[10px] font-bold uppercase mb-1">
                    {isBengali ? "ট্রানজেকশন আইডি (TxnID)" : "Transaction ID (TxnID)"}
                  </label>
                  <input
                    type="text"
                    required
              ...
              <div className="pt-1 flex gap-2.5">
                <button
                  type="button"
                  onClick={() => setShowAddMoneyModal(false)}
                  className="flex-1 py-2.5 bg-zinc-900 hover:bg-zinc-800 border border-white/5 text-zinc-400 rounded-xl text-xs font-bold uppercase tracking-wider transition-colors cursor-pointer"
                >
                ...
                <button
                  ...
                  className="flex-1 py-2.5 bg-emerald-600 hover:bg-emerald-500 disabled:bg-emerald-850 text-white font-black rounded-xl text-xs font-bold uppercase tracking-wider transition-colors cursor-pointer shadow-lg shadow-emerald-950/35"
                >
      ...
      {/* HOW TO PAY POPUP MODAL */}
      {showHowToPayModal && (
        <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-black/85 backdrop-blur-sm animate-fade-in">
          <div className="w-full max-w-sm bg-[#0a0a0a] border border-violet-500/40 rounded-2xl overflow-hidden shadow-2xl p-5 space-y-4 font-mono text-xs">
            <div className="flex items-center justify-between border-b border-white/10 pb-3">
              <h4 className="font-bold text-violet-300 flex items-center gap-1.5 uppercase text-xs">
                <ChevronRight className="w-4 h-4 text-violet-400" />
                <span>{isBengali ? "কীভাবে পেমেন্ট করবেন? (How to Pay)" : "How to Pay Instructions"}</span>
              </h4>
              <button
                onClick={() => setShowHowToPayModal(false)}
            ...
            {(() => {
              ...
              if (customRules && customRules.trim()) {
                return (
                  <div className="text-zinc-300 space-y-2 text-xs leading-relaxed font-sans bg-black/50 p-3 rounded-xl border border-white/5">
              ...
              return (
                <div className="space-y-2 text-zinc-300 font-sans leading-relaxed text-xs">
                  <p>1. {isBengali ? "আপনার পার্সোনাল বিকাশ/নগদ অ্যাপ খুলুন।" : "Open your personal bKash or Nagad App."}</p>
                  <p>2. <strong className="text-amber-400">{isBengali ? "সেন্ড মানি (Send Money)" : "Send Money"}</strong> {isBengali ? "অপশন সিলেক্ট করুন।" : "option select করুন।"}</p>
                  <p>3. {isBengali ? "উপরে প্রদর্শিত নম্বরে নির্দিষ্ট পরিমাণ টাকা পাঠান।" : "Send exact amount to the official number shown above."}</p>
                  <p>4. {isBengali ? "টাকা পাঠানো শেষে প্রাপ্ত Transaction ID (TxnID) এবং আপনার নম্বর ফর্মে দিয়ে জমা দিন।" : "After sending, copy the Transaction ID (TxnID) and submit it in the form."}</p>
                </div>
              );
            })()}
            ...
            <button
              type="button"
              onClick={() => setShowHowToPayModal(false)}
              className="w-full py-2 bg-violet-600 hover:bg-violet-500 text-white font-bold rounded-xl transition-colors cursor-pointer uppercase text-xs"
            >
