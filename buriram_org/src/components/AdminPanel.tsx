...
import { 
  createTournament, 
  updateTournament, 
  updateBatchTournamentWhatsAppLinks,
  deleteTournament, 
  resetDailyMatches, 
...
export default function AdminPanel({ tournaments, transactions, siteSettings, isBengali }: AdminPanelProps) {
  const [activeSubTab, setActiveSubTab] = useState<"tournaments" | "transactions" | "settings" | "rules" | "quick_links">("tournaments");

  // Quick CG Links Management State
  const [cgLinksMap, setCgLinksMap] = useState<Record<string, string>>({});
  const [isSavingCgLinks, setIsSavingCgLinks] = useState(false);
  const [cgLinksMessage, setCgLinksMessage] = useState("");
  const [cgSearchQuery, setCgSearchQuery] = useState("");
  const [cgCategoryFilter, setCgCategoryFilter] = useState<string>("All");

  // Sync CG links state when tournaments prop updates
  useEffect(() => {
    const map: Record<string, string> = {};
    tournaments.forEach(t => {
      map[t.id] = t.whatsAppLink ? decryptLink(t.whatsAppLink) : "";
    });
    setCgLinksMap(map);
  }, [tournaments]);
  ...
  const [overlayCategoryTab, setOverlayCategoryTab] = useState<"Champion Rush" | "Scrims" | "Paid Tournaments">("Champion Rush");

  const [crBadge1, setCrBadge1] = useState(siteSettings.championRushOverlay?.badge1Text || "ফাস্ট পেইস ম্যাচ");
  const [crBadge2, setCrBadge2] = useState(siteSettings.championRushOverlay?.badge2Text || "ফ্রি ফায়ার");
  const [crTitle, setCrTitle] = useState(siteSettings.championRushOverlay?.title || "চ্যাম্পিয়ন রাশ");
  const [crSubtitle, setCrSubtitle] = useState(siteSettings.championRushOverlay?.subtitle || "তাত্ক্ষণিক বুকিং এবং সরাসরি ফাস্ট-পেস ম্যাচ খেলা।");
  const [crButtonText, setCrButtonText] = useState(siteSettings.championRushOverlay?.buttonText || "বুক স্লট");

  const [scrimsBadge1, setScrimsBadge1] = useState(siteSettings.scrimsOverlay?.badge1Text || "অফিসিয়াল টুর্নামেন্ট");
  const [scrimsBadge2, setScrimsBadge2] = useState(siteSettings.scrimsOverlay?.badge2Text || "ফ্রি ফায়ার");
  const [scrimsTitle, setScrimsTitle] = useState(siteSettings.scrimsOverlay?.title || "স্ক্রিমস টুর্নামেন্ট");
  const [scrimsSubtitle, setScrimsSubtitle] = useState(siteSettings.scrimsOverlay?.subtitle || "আপনার গিল্ডের যোগ্যতা প্রমাণ করার গ্লোবাল প্ল্যাটফর্ম।");
  const [scrimsButtonText, setScrimsButtonText] = useState(siteSettings.scrimsOverlay?.buttonText || "বুক স্লট");

  const [paidBadge1, setPaidBadge1] = useState(siteSettings.paidOverlay?.badge1Text || "প্রাইজপুল টুর্নামেন্ট");
  const [paidBadge2, setPaidBadge2] = useState(siteSettings.paidOverlay?.badge2Text || "ফ্রি ফায়ার");
  const [paidTitle, setPaidTitle] = useState(siteSettings.paidOverlay?.title || "পেইড টুর্নামেন্ট");
  const [paidSubtitle, setPaidSubtitle] = useState(siteSettings.paidOverlay?.subtitle || "বড় পুরস্কার জিতার সুবর্ণ সুযোগ, যোগ দিন আজই!");
  const [paidButtonText, setPaidButtonText] = useState(siteSettings.paidOverlay?.buttonText || "বুক স্লট");
  ...
  useEffect(() => {
    ...
    setCrBadge1(siteSettings.championRushOverlay?.badge1Text || "ফাস্ট পেইস ম্যাচ");
    setCrBadge2(siteSettings.championRushOverlay?.badge2Text || "ফ্রি ফায়ার");
    setCrTitle(siteSettings.championRushOverlay?.title || "চ্যাম্পিয়ন রাশ");
    setCrSubtitle(siteSettings.championRushOverlay?.subtitle || "তাত্ক্ষণিক বুকিং এবং সরাসরি ফাস্ট-পেস ম্যাচ খেলা।");
    setCrButtonText(siteSettings.championRushOverlay?.buttonText || "বুক স্লট");

    setScrimsBadge1(siteSettings.scrimsOverlay?.badge1Text || "অফিসিয়াল টুর্নামেন্ট");
    setScrimsBadge2(siteSettings.scrimsOverlay?.badge2Text || "ফ্রি ফায়ার");
    setScrimsTitle(siteSettings.scrimsOverlay?.title || "স্ক্রিমস টুর্নামেন্ট");
    setScrimsSubtitle(siteSettings.scrimsOverlay?.subtitle || "আপনার গিল্ডের যোগ্যতা প্রমাণ করার গ্লোবাল প্ল্যাটফর্ম।");
    setScrimsButtonText(siteSettings.scrimsOverlay?.buttonText || "বুক স্লট");

    setPaidBadge1(siteSettings.paidOverlay?.badge1Text || "প্রাইজপুল টুর্নামেন্ট");
    setPaidBadge2(siteSettings.paidOverlay?.badge2Text || "ফ্রি ফায়ার");
    setPaidTitle(siteSettings.paidOverlay?.title || "পেইড টুর্নামেন্ট");
    setPaidSubtitle(siteSettings.paidOverlay?.subtitle || "বড় পুরস্কার জিতার সুবর্ণ সুযোগ, যোগ দিন আজই!");
    setPaidButtonText(siteSettings.paidOverlay?.buttonText || "বুক স্লট");
  }, [siteSettings]);
  ...
  const handleSettingsSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await updateSiteSettings({
        ...
        championRushOverlay: {
          badge1Text: crBadge1.trim(),
          badge2Text: crBadge2.trim(),
          title: crTitle.trim(),
          subtitle: crSubtitle.trim(),
          buttonText: crButtonText.trim()
        },
        scrimsOverlay: {
          badge1Text: scrimsBadge1.trim(),
          badge2Text: scrimsBadge2.trim(),
          title: scrimsTitle.trim(),
          subtitle: scrimsSubtitle.trim(),
          buttonText: scrimsButtonText.trim()
        },
        paidOverlay: {
          badge1Text: paidBadge1.trim(),
          badge2Text: paidBadge2.trim(),
          title: paidTitle.trim(),
          subtitle: paidSubtitle.trim(),
          buttonText: paidButtonText.trim()
        },
        bannerBadge1Text: scrimsBadge1.trim(),
        bannerBadge2Text: scrimsBadge2.trim(),
        bannerOverlayTitle: scrimsTitle.trim(),
        bannerOverlaySubtitle: scrimsSubtitle.trim(),
        bannerOverlayButtonText: scrimsButtonText.trim()
      });
  ...
  const handleSaveAllCgLinks = async () => {
    setIsSavingCgLinks(true);
    setCgLinksMessage("");
    try {
      const encryptedMap: Record<string, string> = {};
      Object.entries(cgLinksMap).forEach(([id, rawUrl]) => {
        const strVal = typeof rawUrl === "string" ? rawUrl.trim() : "";
        encryptedMap[id] = strVal ? encryptLink(strVal) : "";
      });
      await updateBatchTournamentWhatsAppLinks(encryptedMap);
      setCgLinksMessage(isBengali ? "সকল হোয়াটসঅ্যাপ সিজি গ্রুপ লিংক সফলভাবে আপডেট করা হয়েছে!" : "All WhatsApp CG Links updated successfully!");
      setTimeout(() => setCgLinksMessage(""), 4000);
    } catch (err: any) {
      setCgLinksMessage(err.message || "Failed to update WhatsApp links.");
    } finally {
      setIsSavingCgLinks(false);
    }
  };
  ...
  return (
    <div className="space-y-8 animate-fade-in font-mono text-xs">
      ...
      <div className="flex flex-wrap border-b border-zinc-800 gap-1 sm:gap-2">
        <button
        ...
        <button
          onClick={() => setActiveSubTab("quick_links")}
          className={`px-4 py-2 font-bold uppercase transition-all tracking-wider flex items-center gap-1.5 ${
            activeSubTab === "quick_links" 
              ? "text-emerald-400 border-b-2 border-emerald-500 pb-1.5 font-black" 
              : "text-zinc-400 hover:text-zinc-200 pb-1.5"
          }`}
        >
          <span className="w-2 h-2 rounded-full bg-emerald-500 animate-pulse"></span>
          {isBengali ? "কুইক সিজি লিংকস" : "Quick CG Links"}
        </button>
        <button
      ...
      {/* ==================== SUBTAB: QUICK CG LINKS MANAGEMENT ==================== */}
      {activeSubTab === "quick_links" && (
        <div className="space-y-6 animate-fade-in font-sans">
          {/* Header Card */}
          <div className="bg-gradient-to-r from-zinc-900 via-zinc-900/90 to-emerald-950/40 border border-emerald-500/30 rounded-2xl p-5 md:p-6 shadow-xl flex flex-col md:flex-row md:items-center justify-between gap-4">
            <div className="space-y-1 max-w-2xl">
              <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-emerald-500/10 border border-emerald-500/30 text-emerald-400 font-mono text-[10px] font-bold uppercase tracking-wider">
                <span className="w-2 h-2 rounded-full bg-emerald-400 animate-ping" />
                {isBengali ? "একক ক্লিক কুইক আপডেট" : "One-Click Quick Sync"}
              </span>
              <h3 className="text-lg md:text-xl font-black text-white uppercase tracking-tight font-sans">
                {isBengali ? "কুইক সিজি (WhatsApp Group Link) ম্যানেজমেন্ট" : "Quick CG (WhatsApp Group Link) Manager"}
              </h3>
              <p className="text-xs text-zinc-300 leading-relaxed font-sans" style={{ fontFamily: "'Hind Siliguri', sans-serif" }}>
                {isBengali 
                  ? "ওয়েবসাইটে থাকা সকল একটিভ স্ক্রিম, চ্যাম্পিয়ন রাশ এবং টুর্নামেন্টের কাস্টম হোয়াটসঅ্যাপ গ্রুপ লিংক দ্রুত একসাথে আপডেট বা পরিমার্জন করুন।" 
                  : "Quickly manage and update WhatsApp group links for all active matches in one centralized page."}
              </p>
            </div>
            ...
            <button
              type="button"
              onClick={handleSaveAllCgLinks}
              disabled={isSavingCgLinks}
              className="px-6 py-3 bg-emerald-600 hover:bg-emerald-500 active:scale-95 disabled:bg-emerald-800/50 text-white font-mono text-xs font-black uppercase tracking-wider rounded-xl transition-all cursor-pointer shadow-lg shadow-emerald-950/50 border border-emerald-400/30 flex items-center justify-center gap-2 shrink-0"
            >
              {isSavingCgLinks ? (
                <>
                  <span className="w-4 h-4 border-2 border-white/30 border-t-white rounded-full animate-spin" />
                  <span>{isBengali ? "সেভ হচ্ছে..." : "Saving All..."}</span>
                </>
              ) : (
                <>
                  <span>💾</span>
                  <span>{isBengali ? "সমস্ত লিংক সেভ করুন (Save All Links)" : "Save All Links"}</span>
                </>
              )}
            </button>
          </div>
          ...
          {cgLinksMessage && (
            <div className="p-4 bg-emerald-950/40 border border-emerald-500/40 text-emerald-300 rounded-xl text-xs font-mono font-bold text-center shadow-lg animate-bounce">
              {cgLinksMessage}
            </div>
          )}

          {/* Reset Notice Banner */}
          <div className="p-4 rounded-xl bg-violet-950/30 border border-violet-500/30 text-violet-300 text-xs font-mono flex items-start sm:items-center gap-3">
            <span className="text-base shrink-0">ℹ️</span>
            <p className="leading-relaxed font-sans" style={{ fontFamily: "'Hind Siliguri', sans-serif" }}>
              <strong className="text-violet-200">
                {isBengali ? "অটো-রিসেট নিয়ম:" : "Auto-Reset Policy:"}
              </strong>{" "}
              {isBengali
                ? "প্রতিদিন রাত ১:০০ AM অটোমেটিক রিসেটে অথবা অ্যাডমিনের 'ম্যানুয়াল রিসেট' বাটনে ক্লিকে স্লট খালি হওয়ার পাশাপাশি সকল টুর্নামেন্টের বিদ্যমান হোয়াটসঅ্যাপ গ্রুপ লিংক স্বয়ংক্রিয়ভাবে মুছে খালি (Reset to Null) হয়ে যাবে।"
                : "Every day at 1:00 AM automatic reset or manual reset will automatically clear and empty all tournament WhatsApp CG links."}
            </p>
          </div>
          ...
          <div className="flex flex-col sm:flex-row items-center justify-between gap-3 bg-zinc-900/60 p-3.5 rounded-xl border border-zinc-800">
            ...
            <div className="w-full sm:w-64">
              <input
                ...
                placeholder={isBengali ? "টুর্নামেন্ট ফিল্টার করুন..." : "Search tournament title..."}
                className="w-full px-3 py-1.5 bg-zinc-950 border border-zinc-800 rounded-lg text-white font-mono text-xs focus:outline-none focus:border-violet-500"
              />
            </div>
          </div>

          {/* List of Tournaments with CG Link Input */}
          {(() => {
            const filteredTournaments = tournaments.filter((t) => {
              const matchCat = cgCategoryFilter === "All" || t.category === cgCategoryFilter;
              const matchQuery = !cgSearchQuery.trim() || t.title.toLowerCase().includes(cgSearchQuery.toLowerCase());
              ...
              return (
                <div className="p-12 text-center bg-zinc-900/30 border border-zinc-800 rounded-2xl">
                  <p className="text-zinc-500 font-mono text-xs">
                    {isBengali ? "কোনো টুর্নামেন্ট বা স্ক্রিম ম্যাচ পাওয়া যায়নি।" : "No matches found matching criteria."}
                  </p>
                </div>
              );
            }
            ...
            return (
              <div className="space-y-3.5">
                {filteredTournaments.map((t) => {
                  const rawUrl = cgLinksMap[t.id] || "";
                  const isModified = t.whatsAppLink ? decryptLink(t.whatsAppLink) !== rawUrl : rawUrl.trim() !== "";
                    ...
                    >
                      ...
                      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 border-b border-zinc-800/60 pb-2.5">
                        <div className="flex items-center gap-2 flex-wrap">
                          ...
                          <h4 className="font-bold text-white font-sans text-sm tracking-tight uppercase">
                            {t.title}
                          </h4>
                          {t.mapName && (
                            <span className="text-[10px] text-zinc-400 font-mono bg-zinc-950 border border-zinc-800 px-1.5 py-0.5 rounded">
                              🗺️ {t.mapName}
                            </span>
                          )}
                        </div>
                        ...
                        <div className="flex items-center gap-3 text-[10px] font-mono text-zinc-400">
                          <span>
                            ⏰ {t.startDateTime ? t.startDateTime.replace("T", " ") : "N/A"}
                          </span>
                          <span className="text-zinc-600">•</span>
                          <span className={t.takenSlots >= t.totalSlots ? "text-rose-400 font-bold" : "text-emerald-400 font-bold"}>
                            👥 {t.takenSlots} / {t.totalSlots} Slots
                          </span>
                        </div>
                      </div>

                      {/* WhatsApp CG Link Input Line */}
                      <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-2">
                        <div className="relative flex-1">
                          ...
                          <input
                            ...
                            onChange={(e) => {
                              const val = e.target.value;
                              setCgLinksMap((prev) => ({ ...prev, [t.id]: val }));
                            }}
                            placeholder="https://chat.whatsapp.com/..."
                            className="w-full pl-9 pr-8 py-2.5 bg-zinc-950 border border-zinc-800 rounded-xl text-white font-mono text-xs focus:outline-none focus:border-emerald-500 transition-colors"
                          />
                          {rawUrl && (
                            <button
                              type="button"
                              onClick={() => setCgLinksMap((prev) => ({ ...prev, [t.id]: "" }))}
                              className="absolute right-2.5 top-1/2 -translate-y-1/2 text-zinc-500 hover:text-rose-400 font-bold p-1 cursor-pointer"
                              title="Clear link"
                            >
                              ✕
                            </button>
                        ...
                        <div className="flex items-center gap-2 shrink-0">
                          {rawUrl.trim() ? (
                            <a
                              ...
                              className="px-3 py-2 bg-emerald-950/60 hover:bg-emerald-900 border border-emerald-500/30 text-emerald-300 font-mono text-[10px] font-bold uppercase rounded-lg transition-all flex items-center gap-1"
                            >
                              <span>🔗</span>
                              <span>{isBengali ? "টেস্ট করুন" : "Test Link"}</span>
                            </a>
                          ) : (
                            <span className="px-3 py-2 bg-zinc-950 border border-zinc-800 text-zinc-500 font-mono text-[10px] rounded-lg">
                              ⚪ {isBengali ? "খালি" : "Empty"}
                            </span>
                          )}
                        </div>
                      </div>
                ...
                {/* Bottom Save All Links Floating Bar */}
                <div className="pt-4 flex justify-end">
                  <button
                    type="button"
                    onClick={handleSaveAllCgLinks}
                    disabled={isSavingCgLinks}
                    className="w-full sm:w-auto px-8 py-3.5 bg-emerald-600 hover:bg-emerald-500 active:scale-95 disabled:bg-emerald-800/50 text-white font-mono text-xs font-black uppercase tracking-wider rounded-xl transition-all cursor-pointer shadow-xl shadow-emerald-950/50 border border-emerald-400/30 flex items-center justify-center gap-2"
                  >
                    {isSavingCgLinks ? (
                      <>
                        <span className="w-4 h-4 border-2 border-white/30 border-t-white rounded-full animate-spin" />
                        <span>{isBengali ? "সেভ হচ্ছে..." : "Saving All Links..."}</span>
                      </>
                    ) : (
                      <>
                        <span>💾</span>
            ...
            <div className="space-y-3 pt-2 border-t border-zinc-900">
              <div className="flex items-center gap-2">
                ...
                <label htmlFor="enableBannerTextOverlay" className="text-xs text-zinc-400 font-bold select-none cursor-pointer uppercase">
                  {isBengali ? "হিরো ব্যানারে কাস্টম টেক্সট ওভারলে এবং বাটন সক্রিয় করুন" : "Enable Custom Text Overlay & Button on Hero Banners"}
                </label>
              </div>

              {enableBannerTextOverlay && (
                <div className="space-y-4 p-4 bg-zinc-900/80 border border-zinc-800 rounded-xl">
                  {/* Category Selection Tabs for Banner Overlay */}
                  <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 border-b border-zinc-800/80 pb-3">
                    <span className="text-xs font-bold text-violet-300 uppercase tracking-wide flex items-center gap-1.5">
                      <span>🏷️</span>
                      <span>{isBengali ? "ক্যাটাগরি কাস্টম ওভারলে নির্বাচন করুন:" : "Select Category Overlay Settings:"}</span>
                    </span>

                    <div className="flex flex-wrap gap-1.5">
                      {(["Champion Rush", "Scrims", "Paid Tournaments"] as const).map((cat) => (
                        <button
                          key={cat}
                          type="button"
                          onClick={() => setOverlayCategoryTab(cat)}
                          className={`px-3 py-1.5 rounded-lg text-xs font-mono font-bold uppercase transition-all cursor-pointer border ${
                            overlayCategoryTab === cat
                        ...
                        >
                          {cat === "Champion Rush"
                            ...
                            : cat === "Scrims"
                            ? (isBengali ? "⚔️ স্ক্রিমস টুর্নামেন্ট" : "⚔️ Scrims Match")
                  ...
                  {/* 1. Champion Rush Overlay Inputs */}
                  {overlayCategoryTab === "Champion Rush" && (
                    <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-3 animate-fade-in">
                      <div>
                        <label className="block text-[10px] text-zinc-400 font-bold uppercase mb-1">
                          {isBengali ? "a) Badge 1 Text (ব্যাজ ১)" : "a) Badge 1 Text"}
                        </label>
                        <input
                          type="text"
                          value={crBadge1}
                          onChange={(e) => setCrBadge1(e.target.value)}
                          placeholder={isBengali ? "যেমন: ফাস্ট পেইস ম্যাচ" : "e.g. FAST MATCH"}
                          className="w-full p-2 bg-zinc-950 border border-zinc-800 rounded text-white text-xs font-mono focus:border-violet-500"
                        />
                      </div>

                      <div>
                        <label className="block text-[10px] text-zinc-400 font-bold uppercase mb-1">
                          {isBengali ? "b) Badge 2 Text (ব্যাজ ২)" : "b) Badge 2 Text"}
                        </label>
                        <input
                          type="text"
                          value={crBadge2}
                          onChange={(e) => setCrBadge2(e.target.value)}
                          placeholder={isBengali ? "যেমন: ফ্রি ফায়ার" : "e.g. Free Fire"}
                          className="w-full p-2 bg-zinc-950 border border-zinc-800 rounded text-white text-xs font-mono focus:border-violet-500"
                        />
                      </div>
                      ...
                      <div>
                        <label className="block text-[10px] text-zinc-400 font-bold uppercase mb-1">
                          {isBengali ? "c) Main Title (প্রধান টাইটেল)" : "c) Main Title"}
                        </label>
                        <input
                          type="text"
                          value={crTitle}
                          onChange={(e) => setCrTitle(e.target.value)}
                          placeholder={isBengali ? "যেমন: চ্যাম্পিয়ন রাশ" : "e.g. CHAMPION RUSH"}
                          className="w-full p-2 bg-zinc-950 border border-zinc-800 rounded text-white text-xs font-mono focus:border-violet-500"
                        />
                      </div>
                      ...
                      <div className="sm:col-span-2">
                        <label className="block text-[10px] text-zinc-400 font-bold uppercase mb-1">
                          {isBengali ? "d) Subtitle (সাবটাইটেল / বর্ণনা)" : "d) Subtitle"}
                        </label>
                        <input
                          type="text"
                          value={crSubtitle}
                          onChange={(e) => setCrSubtitle(e.target.value)}
                          placeholder={isBengali ? "যেমন: তাত্ক্ষণিক বুকিং এবং সরাসরি ফাস্ট-পেস ম্যাচ খেলা।" : "e.g. Instant squad booking & action-packed rush lobbies."}
                          className="w-full p-2 bg-zinc-950 border border-zinc-800 rounded text-white text-xs font-mono focus:border-violet-500"
                        />
                      </div>
                      ...
                      <div>
                        <label className="block text-[10px] text-zinc-400 font-bold uppercase mb-1">
                          {isBengali ? "e) Button Text (বাটন টেক্সট - ঐচ্ছিক)" : "e) Button Text (Optional)"}
                        </label>
                        <input
                          type="text"
                          value={crButtonText}
                          onChange={(e) => setCrButtonText(e.target.value)}
                          placeholder={isBengali ? "যেমন: বুক স্লট" : "e.g. BOOK SLOT"}
                          className="w-full p-2 bg-zinc-950 border border-zinc-800 rounded text-white text-xs font-mono focus:border-violet-500"
                        />
                      </div>
                    </div>
                  ...
                  {/* 2. Scrims Overlay Inputs */}
                  {overlayCategoryTab === "Scrims" && (
                    <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-3 animate-fade-in">
                      <div>
                        <label className="block text-[10px] text-zinc-400 font-bold uppercase mb-1">
                          {isBengali ? "a) Badge 1 Text (ব্যাজ ১)" : "a) Badge 1 Text"}
                        ...
                        <input
                          type="text"
                          value={scrimsBadge1}
                          onChange={(e) => setScrimsBadge1(e.target.value)}
                          placeholder={isBengali ? "যেমন: অফিসিয়াল টুর্নামেন্ট" : "e.g. BURIRAM VERIFIED"}
                          className="w-full p-2 bg-zinc-950 border border-zinc-800 rounded text-white text-xs font-mono focus:border-violet-500"
                        />
                      </div>
                      ...
                      <div>
                        ...
                        <input
                          type="text"
                          value={scrimsBadge2}
                          onChange={(e) => setScrimsBadge2(e.target.value)}
                          placeholder={isBengali ? "যেমন: ফ্রি ফায়ার" : "e.g. Free Fire"}
                          className="w-full p-2 bg-zinc-950 border border-zinc-800 rounded text-white text-xs font-mono focus:border-violet-500"
                        />
                      </div>
                      ...
                      <div>
                        ...
                        <input
                          type="text"
                          value={scrimsTitle}
                          onChange={(e) => setScrimsTitle(e.target.value)}
                          placeholder={isBengali ? "যেমন: স্ক্রিমস টুর্নামেন্ট" : "e.g. CYBER SCRIMS"}
                          className="w-full p-2 bg-zinc-950 border border-zinc-800 rounded text-white text-xs font-mono focus:border-violet-500"
                        />
                      </div>
                      ...
                      <div className="sm:col-span-2">
                        ...
                        <input
                          type="text"
                          value={scrimsSubtitle}
                          onChange={(e) => setScrimsSubtitle(e.target.value)}
                          placeholder={isBengali ? "যেমন: আপনার গিল্ডের যোগ্যতা প্রমাণ করার গ্লোবাল প্ল্যাটফর্ম।" : "e.g. Elite tier guild matchmaking & status scoring lobby."}
                          className="w-full p-2 bg-zinc-950 border border-zinc-800 rounded text-white text-xs font-mono focus:border-violet-500"
                        />
                      </div>
                      ...
                      <div>
                        ...
                        <input
                          type="text"
                          value={scrimsButtonText}
                          onChange={(e) => setScrimsButtonText(e.target.value)}
                          placeholder={isBengali ? "যেমন: বুক স্লট" : "e.g. BOOK SLOT"}
                          className="w-full p-2 bg-zinc-950 border border-zinc-800 rounded text-white text-xs font-mono focus:border-violet-500"
                        />
                      </div>
                  ...
                  {/* 3. Paid Tournaments Overlay Inputs */}
                  {overlayCategoryTab === "Paid Tournaments" && (
                    <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-3 animate-fade-in">
                      <div>
                        <label className="block text-[10px] text-zinc-400 font-bold uppercase mb-1">
                          {isBengali ? "a) Badge 1 Text (ব্যাজ ১)" : "a) Badge 1 Text"}
                        ...
                        <input
                          type="text"
                          value={paidBadge1}
                          onChange={(e) => setPaidBadge1(e.target.value)}
                          placeholder={isBengali ? "যেমন: প্রাইজপুল টুর্নামেন্ট" : "e.g. PRIZE POOL"}
                          className="w-full p-2 bg-zinc-950 border border-zinc-800 rounded text-white text-xs font-mono focus:border-violet-500"
                        />
                      </div>
                      ...
                      <div>
                        ...
                        <input
                          type="text"
                          value={paidBadge2}
                          onChange={(e) => setPaidBadge2(e.target.value)}
                          placeholder={isBengali ? "যেমন: ফ্রি ফায়ার" : "e.g. Free Fire"}
                          className="w-full p-2 bg-zinc-950 border border-zinc-800 rounded text-white text-xs font-mono focus:border-violet-500"
                        />
                      </div>
                      ...
                      <div>
                        ...
                        <input
                          ...
                          placeholder={isBengali ? "যেমন: পেইড টুর্নামেন্ট" : "e.g. PAID ESPORTS"}
                          className="w-full p-2 bg-zinc-950 border border-zinc-800 rounded text-white text-xs font-mono focus:border-violet-500"
                        />
                      </div>
                      ...
                      <div className="sm:col-span-2">
                        ...
                        <input
                          type="text"
                          value={paidSubtitle}
                          onChange={(e) => setPaidSubtitle(e.target.value)}
                          placeholder={isBengali ? "যেমন: বড় পুরস্কার জিতার সুবর্ণ সুযোগ, যোগ দিন আজই!" : "e.g. High stakes prize pool tournaments. Book your slot now!"}
                          className="w-full p-2 bg-zinc-950 border border-zinc-800 rounded text-white text-xs font-mono focus:border-violet-500"
                        />
                      </div>
                      ...
                      <div>
                        ...
                        <input
                          type="text"
                          value={paidButtonText}
                          onChange={(e) => setPaidButtonText(e.target.value)}
                          placeholder={isBengali ? "যেমন: বুক স্লট" : "e.g. BOOK SLOT"}
                          className="w-full p-2 bg-zinc-950 border border-zinc-800 rounded text-white text-xs font-mono focus:border-violet-500"
                        />
                      </div>
