...
export default function CountdownTimer({ tournament, isBengali, onFinished }: CountdownTimerProps) {
  ...
  return (
    <div className="inline-flex flex-col items-start gap-0.5 bg-gradient-to-r from-amber-500/15 via-amber-400/20 to-yellow-500/15 border border-amber-400/40 px-3 py-1.5 rounded-xl shadow-[0_0_15px_rgba(251,191,36,0.15)]">
      <span className="text-[9px] font-bold uppercase tracking-wide text-amber-300 font-sans">
        {isBengali ? "ম্যাচ রেজিস্ট্রেশন শেষ হবে:" : "REGISTRATION ENDS IN:"}
      </span>
      <div className="flex items-center gap-1 font-mono text-sm font-black text-amber-300 tracking-wider">
        <span className="w-5 text-center">{hDisplay}</span>
        <span className="text-amber-400 animate-pulse">:</span>
        <span className="w-5 text-center">{mDisplay}</span>
        <span className="text-amber-400 animate-pulse">:</span>
        <span className="w-5 text-center">{sDisplay}</span>
      </div>
    </div>
  );
}
